...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://postgres:siyi1126@localhost/tayzhengHW5"
## Provided:
...
def update(item):
    ...
    if form.validate_on_submit():
        TodoItem.query.filter_by(id = item).first().priority = form.update_priority.data
        item = TodoItem.query.filter_by(id = item).first().description
        db.session.commit()
        flash("You just updated priority of task: " + str(item))
...
def delete(lst):
    ...
    db.session.commit()
    flash(todo_lst.title + " was successfully deleted!")
    return redirect("/all_lists")
